...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <!-- Page Header -->
      <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm">
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">AI Social & Call Automation</h2>
      ...
      <div class="flex border-b border-slate-200 dark:border-slate-700 space-x-6">
        <button 
          ...
          class="pb-3 text-sm font-semibold border-b-2 border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 transition-colors flex items-center gap-2">
      ...
      <div *ngIf="activeTab() === 'caller'" class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm p-6 space-y-4">
            ...
            <!-- Template selection -->
            <div>
              <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Load Call Demo Templates</label>
              <div class="grid grid-cols-1 gap-2">
                <button 
                  type="button"
                  *ngFor="let t of callTemplates"
                  (click)="loadCallTemplate(t.text)"
                  class="p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 hover:bg-slate-100 dark:bg-slate-900/50 dark:hover:bg-slate-900 text-left text-xs font-semibold text-slate-700 dark:text-slate-300 hover:border-sky-400 transition-all flex items-start gap-2">
                  <span class="material-icons text-sky-500 text-sm mt-0.5">description</span>
                  <div class="min-w-0 flex-1">
                    <p class="truncate">{{ t.label }}</p>
                    <span class="text-[9px] text-slate-400 block">{{ t.description }}</span>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div class="space-y-1.5">
              <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Paste Call Transcript</label>
              <textarea 
                rows="8"
                [(ngModel)]="transcript" 
        ...
        <div class="lg:col-span-3">
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm p-6 min-h-[400px] flex flex-col justify-between">
            
            <div class="space-y-6">
              <h3 class="font-bold text-slate-800 dark:text-white text-sm border-b border-slate-100 dark:border-slate-700 pb-3 flex justify-between items-center">
              ...
              <!-- Loading Indicator -->
              <div *ngIf="isLoading()" class="py-24 text-center text-slate-400">
              ...
              <!-- Empty State -->
              <div *ngIf="!analysis() && !isLoading()" class="py-24 text-center text-slate-400">
                <span class="material-icons text-4xl mb-2 text-slate-300">analytics</span>
              ...
              <div *ngIf="analysis() as data">
                <div *ngIf="!isLoading()" class="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <!-- Sentiment Card -->
                    <div class="p-4 rounded-xl border border-slate-100 dark:border-slate-700/40 bg-slate-50/50 dark:bg-slate-900/30 flex items-center gap-3">
                      <div class="h-10 w-10 rounded-full flex items-center justify-center shrink-0"
                        [ngClass]="{
                          ...
                          'bg-slate-100 text-slate-600 dark:bg-slate-900 dark:text-slate-400': data.sentiment === 'Neutral'
                        }">
                      ...
                      <div>
                        <span class="text-[9px] text-slate-400 uppercase font-bold tracking-wider">Detected Sentiment</span>
                        <h4 class="text-base font-bold text-slate-800 dark:text-white"
                          [ngClass]="{
                            ...
                            'text-slate-500': data.sentiment === 'Neutral'
                          }">{{ data.sentiment }}</h4>
                      </div>
                    </div>
                    ...
                    <div class="p-4 rounded-xl border border-slate-100 dark:border-slate-700/40 bg-slate-50/50 dark:bg-slate-900/30 flex items-center gap-3">
                      ...
                      <div>
                        <span class="text-[9px] text-slate-400 uppercase font-bold tracking-wider">Analysis Confidence</span>
                        <h4 class="text-base font-bold text-slate-800 dark:text-white">96% (Gemini Verified)</h4>
                      </div>
                    </div>
                  ...
                  <!-- Executive Summary -->
                  <div class="p-4 rounded-xl border border-slate-100 dark:border-slate-700/40 bg-slate-50/50 dark:bg-slate-900/30">
                    <h4 class="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider mb-2">Executive Summary</h4>
                  ...
                  <div>
                    <h4 class="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    <div class="space-y-2">
                      <div 
                        *ngFor="let item of data.actionItems; let i = index"
                        class="flex items-start gap-3 p-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50/20 dark:bg-slate-900/20 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                        <input 
                          type="checkbox" 
                          [id]="'action-' + i"
                          class="mt-0.5 h-4 w-4 text-sky-500 focus:ring-sky-500 border-slate-300 rounded shrink-0">
                        <label [for]="'action-' + i" class="text-xs text-slate-700 dark:text-slate-300 font-medium cursor-pointer">
                          {{ item }}
                        </label>
                      </div>
                    </div>
            ...
            <div *ngIf="analysis() && !isLoading()" class="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-[11px] text-slate-400">
              ...
              <button 
                (click)="clearAnalysis()"
                class="text-slate-500 hover:text-sky-500 font-semibold uppercase tracking-wider">
                Clear Results
              </button>
            </div>
      ...
      <div *ngIf="activeTab() === 'social'" class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm p-6 space-y-4">
            ...
            <!-- Choose channel -->
            <div class="space-y-2">
              <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Active Channel</label>
              <div class="grid grid-cols-2 gap-2">
                <button 
                  ...
                  class="p-3 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2">
            ...
            <div class="flex items-center justify-between p-3 rounded-lg border border-slate-100 dark:border-slate-700/60 bg-slate-50/50 dark:bg-slate-900/40">
              <div>
                <h5 class="text-xs font-bold text-slate-800 dark:text-white">Instant Auto-Reply</h5>
                <p class="text-[10px] text-slate-400 mt-0.5">Let AI reply immediately to webhooks.</p>
              </div>
            ...
            <!-- Tone selector -->
            <div class="space-y-1.5">
              <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">AI Suggested Tone</label>
              <select 
                [(ngModel)]="aiTone"
            ...
            <div class="border-t border-slate-100 dark:border-slate-700 pt-4 space-y-3">
              <h4 class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Simulate Incoming Message</h4>
              <div class="grid grid-cols-1 gap-1.5">
                <button 
                  type="button"
                  *ngFor="let sample of socialMsgTemplates"
                  (click)="simulateIncomingMessage(sample)"
                  class="px-3 py-2 border border-slate-100 dark:border-slate-700 bg-slate-50/60 hover:bg-slate-100 dark:bg-slate-900/30 text-left text-xs text-slate-600 dark:text-slate-400 rounded-lg hover:border-sky-500 truncate transition-all">
                  "{{ sample }}"
                </button>
              </div>
            </div>
        ...
        <div class="lg:col-span-3">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-lg overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700/60 flex items-center justify-between"
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h4 class="text-sm font-bold text-slate-800 dark:text-white">Grownox Technologies Live Agent</h4>
              ...
              <div class="flex items-center gap-2">
                <span class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
                <span class="text-[10px] font-bold text-slate-400 uppercase">Webhook Active</span>
              </div>
            </div>

            <!-- Chat Message Feed -->
            <div class="flex-grow p-6 overflow-y-auto space-y-4 max-h-[340px] min-h-[340px] bg-slate-50/50 dark:bg-slate-950/30 flex flex-col justify-end">
              
              <!-- Empty state message if no chats -->
              <div *ngIf="chatHistory().length === 0" class="text-center py-12 text-slate-400 my-auto">
                <span class="material-icons text-4xl mb-1 text-slate-300">chat_bubble_outline</span>
                <p class="text-xs font-semibold">Ready for incoming webhook simulation.</p>
                <p class="text-[10px] mt-1 text-slate-400">Click a message template in the sidebar to simulate.</p>
              </div>
              ...
              <div 
                *ngFor="let msg of chatHistory()"
                ...
                <div 
                  ...
                  [class.dark:bg-slate-800]="msg.sender === 'customer'"
                  ...
                  [class.dark:border-slate-700/60]="msg.sender === 'customer'"
            ...
            <div class="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/30 flex gap-2">
              <input 
                type="text" 
                [(ngModel)]="agentInput" 
                placeholder="Type a custom message as client customer..."
                (keyup.enter)="sendAgentMessage()"
                class="flex-grow px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-xs focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-slate-700 dark:text-slate-200">
