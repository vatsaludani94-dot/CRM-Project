  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-extrabold text-slate-800 text-[#1c1917] tracking-tight">Website Builder</h1>
          <p class="text-sm text-[#574c43] mt-1">Design responsive landing pages, update SEO parameters, and bind domains.</p>
        </div>
      ...
      <div *ngIf="activeView() === 'list'" class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fadeIn">
        <div *ngFor="let site of websites()" class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4">
          <div class="space-y-2">
            <div class="flex justify-between items-center">
              ...
              <span [ngClass]="{
                ...
                'bg-slate-100 text-[#292524] border-slate-200': !site.published
              }" class="border px-2 py-0.5 rounded-full text-[9px] font-black uppercase">
                {{ site.published ? 'Published' : 'Draft' }}
              </span>
            </div>
            <h3 class="text-lg font-bold text-slate-900 text-[#1c1917] pt-2">{{ site.name }}</h3>
            <p class="text-xs text-[#44403c] font-medium">Domain: {{ site.domain || (site.subdomain + '.grownx.site') }}</p>
          </div>
        ...
        <div *ngIf="websites().length === 0" class="col-span-3 text-center py-16 bg-white bg-white border border-dashed border-slate-300 border-[#e7e5e4] rounded-2xl space-y-3">
          <span class="material-icons text-4xl text-[#1c1917]">web</span>
          <p class="text-sm font-semibold text-[#574c43]">No websites created yet. Start by choosing a template.</p>
        </div>
      </div>
      ...
      <div *ngIf="activeView() === 'editor' && currentSite()" class="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn">
          ...
          <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl flex flex-col space-y-6 min-h-[500px]">
            ...
            <div class="flex justify-between items-center border-b border-slate-800 pb-4">
              <div class="flex items-center gap-3">
                <button (click)="closeEditor()" class="text-[#44403c] hover:text-white">
              ...
              <div [ngClass]="{
                ...
                <div *ngFor="let sec of currentSiteSections(); let idx = index" (click)="selectSection(idx)" [class.ring-2]="selectedSectionIdx() === idx" class="ring-indigo-500 ring-offset-2 p-4 border border-dashed border-slate-200 hover:border-indigo-400 rounded-xl relative cursor-pointer group transition-all">
                  ...
                  <div class="absolute right-3 top-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                    <button (click)="moveSectionUp(idx); $event.stopPropagation()" [disabled]="idx === 0" class="p-1 bg-slate-100 hover:bg-slate-200 bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-[#1c1917] rounded disabled:opacity-30">
                      <span class="material-icons text-xs">arrow_upward</span>
                    </button>
                    <button (click)="moveSectionDown(idx); $event.stopPropagation()" [disabled]="idx === currentSiteSections().length - 1" class="p-1 bg-slate-100 hover:bg-slate-200 bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-[#1c1917] rounded disabled:opacity-30">
                  ...
                  <div class="space-y-1.5 text-slate-800 dark:text-slate-800">
                    ...
                    <p class="text-[10px] text-[#44403c] font-semibold" *ngIf="sec.subtitle">{{ sec.subtitle }}</p>
                  </div>
                  
                </div>

                <!-- Empty Section State -->
                <div *ngIf="currentSiteSections().length === 0" class="text-center py-12 text-[#44403c] text-xs">
        ...
        <div class="lg:col-span-4 space-y-6">
          ...
          <div *ngIf="activeEditorTab() === 'section'" class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-4">
            <h4 class="font-extrabold text-sm text-slate-700 text-[#1c1917] uppercase tracking-wider flex items-center gap-2">
              <span class="material-icons text-indigo-500">edit</span> Block Settings
            </h4>

            <div *ngIf="selectedSectionIdx() === null" class="text-center py-16 text-[#44403c] text-xs">
            ...
            <div *ngIf="selectedSectionIdx() !== null" class="space-y-4 text-xs animate-fadeIn">
              <div>
                <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Header Title</label>
                <input type="text" [(ngModel)]="getSelectedSection()!.title" class="w-full bg-slate-50 bg-white border border-slate-205 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]">
              </div>
              <div *ngIf="getSelectedSection()!.subtitle !== undefined">
                <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Subtitle / Body</label>
                <textarea [(ngModel)]="getSelectedSection()!.subtitle" rows="3" class="w-full bg-slate-50 bg-white border border-slate-205 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]"></textarea>
              </div>

              <!-- Universal Clickable Actions Configurator -->
              <div class="border-t border-slate-105 border-[#e7e5e4] pt-4 mt-2 space-y-3" *ngIf="getSelectedSection()!.action">
                <h5 class="font-extrabold text-[10px] uppercase tracking-wider text-[#44403c]">Component Action Binding</h5>
                <p class="text-[9px] text-[#44403c] leading-normal">Bind a click trigger action to this section's buttons or cards.</p>
                
                <div>
                  <label class="block text-[9px] font-bold text-[#44403c] uppercase mb-1">Trigger Action Type</label>
                  <select [(ngModel)]="getSelectedSection()!.action!.type" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-900 text-[#1c1917]">
                ...
                <div *ngIf="getSelectedSection()!.action!.type !== 'none'">
                  <label class="block text-[9px] font-bold text-[#44403c] uppercase mb-1">Action Target (URL/Path/ID)</label>
                  <input type="text" [(ngModel)]="getSelectedSection()!.action!.target" placeholder="e.g. /workflows, https://google.com, form_id" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-900 text-[#1c1917]">
                </div>
              </div>
          ...
          <div *ngIf="activeEditorTab() === 'presets'" class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-4 text-xs animate-fadeIn">
            <h4 class="font-extrabold text-sm text-slate-700 text-[#1c1917] uppercase tracking-wider mb-2">Component Presets Library</h4>
            ...
            <div class="space-y-2 max-h-[350px] overflow-y-auto pr-1">
              <div *ngFor="let comp of getFilteredComponents()" (click)="appendPresetComponent(comp)" class="border border-slate-100 border-[#e7e5e4]/60 p-2.5 rounded-xl cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-500 transition-colors flex items-center justify-between group">
                <div>
                  <h5 class="font-bold text-slate-800 text-[#1c1917] text-[10px] group-hover:text-indigo-500 transition-colors">{{ comp.name }}</h5>
                  <span class="text-[8px] bg-slate-100 bg-[#fafaf9] text-[#44403c] px-1 py-0.5 rounded font-black uppercase inline-block mt-1">{{ comp.category }} Preset</span>
                </div>
                <span class="material-icons text-[#1c1917] group-hover:text-indigo-500 transition-colors text-sm">add_circle_outline</span>
              </div>
            </div>
          </div>

          <!-- Editor Panel: SEO & Domains Form -->
          <div *ngIf="activeEditorTab() === 'settings'" class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-5 text-xs">
            <h4 class="font-extrabold text-sm text-slate-700 text-[#1c1917] uppercase tracking-wider mb-2">Publish Settings</h4>
            
            <div class="space-y-3">
              <div>
                <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">SEO Title</label>
                <input type="text" [(ngModel)]="currentSite().seo.title" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]">
              </div>
              <div>
                <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">SEO Meta Description</label>
                <textarea [(ngModel)]="currentSite().seo.description" rows="3" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]"></textarea>
              </div>
              <div>
                <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Custom Domain Name</label>
                <input type="text" [(ngModel)]="customDomainInput" placeholder="www.mybusiness.com" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]">
              </div>
            </div>

            <div class="flex items-center justify-between pt-4 border-t border-slate-100 border-[#e7e5e4]">
              <span class="font-bold text-[#44403c]">Status: {{ currentSite().published ? 'Published' : 'Draft' }}</span>
      ...
      <div *ngIf="showCreateModal()" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white bg-white border border-[#e7e5e4] p-6 rounded-2xl w-full max-w-lg space-y-4 text-xs animate-fadeIn">
          <h3 class="text-sm font-extrabold text-slate-800 text-[#1c1917]">Create New Website Template</h3>
          
          <div class="space-y-3">
            <div>
              <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Website Name</label>
              <input type="text" [(ngModel)]="newSiteName" placeholder="My Startup Portal" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-900 text-[#1c1917]">
            </div>
