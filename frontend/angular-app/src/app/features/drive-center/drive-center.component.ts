  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-extrabold text-slate-800 text-[#1c1917] tracking-tight">Drive Synchronization</h1>
          <p class="text-sm text-[#574c43] mt-1">Manage project assets, client contract files, and folder directories permissions.</p>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs font-semibold text-[#44403c] bg-white bg-white px-4 py-3 rounded-xl border border-slate-200 border-[#e7e5e4]/60 shadow-sm">
        ...
        <span *ngIf="currentFolderName()" class="flex items-center gap-2">
          <span class="material-icons text-[#44403c] text-xs">chevron_right</span>
          <span class="text-slate-800 text-[#1c1917]">{{ currentFolderName() }}</span>
        </span>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn">
        ...
        <div class="lg:col-span-8 space-y-6">
          <div class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-4">
            <h4 class="font-extrabold text-xs text-[#44403c] uppercase tracking-widest">Directories</h4>
            
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div *ngFor="let folder of currentSubfolders()" (click)="navigateToFolder(folder)" class="border border-slate-200 border-[#e7e5e4] bg-slate-50/50 bg-[#fafaf9]/30 p-4 rounded-xl flex items-center gap-3 hover:border-indigo-500 dark:hover:border-indigo-500 cursor-pointer group transition-all">
                <span class="material-icons text-amber-400 group-hover:scale-105 transition-transform">folder</span>
                <div class="overflow-hidden">
                  <h5 class="text-xs font-bold text-slate-800 text-[#1c1917] truncate">{{ folder.name }}</h5>
                  <p class="text-[9px] text-[#44403c] font-bold uppercase">{{ folder.type }}</p>
                </div>
              </div>
              <div *ngIf="currentSubfolders().length === 0" class="col-span-3 text-center py-6 text-xs text-[#44403c] font-semibold">
          ...
          <!-- Files Panel -->
          <div class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-4">
            <h4 class="font-extrabold text-xs text-[#44403c] uppercase tracking-widest">Files</h4>

            <div class="divide-y divide-slate-100 dark:divide-slate-700">
              <div *ngFor="let file of currentFiles()" class="py-4 flex justify-between items-center group">
                <div class="flex items-center gap-3">
                  <span class="material-icons text-[#44403c]" [ngClass]="{
                  ...
                  <div>
                    <h5 class="text-xs font-bold text-slate-800 text-[#1c1917]">{{ file.name }}</h5>
                    <p class="text-[10px] text-[#44403c]">
                ...
                <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button (click)="previewFile(file)" class="bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-700 text-[#44403c] text-xs font-bold px-3 py-1.5 rounded-lg transition-colors">
                    Preview
                  </button>
              ...
              <div *ngIf="currentFiles().length === 0" class="text-center py-12 text-xs text-[#44403c] font-semibold">
        ...
        <div class="lg:col-span-4 bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-6 rounded-2xl shadow-sm space-y-6">
          <h4 class="font-extrabold text-sm text-slate-700 text-[#1c1917] uppercase tracking-wider">Storage Metrics</h4>
          
          <div class="space-y-4 text-xs font-semibold text-[#574c43]">
            <div>
              <div class="flex justify-between mb-1.5">
                <span>Enterprise Space used</span>
                <span class="text-slate-900 text-[#1c1917]">12.4 MB / 10 GB</span>
              </div>
            ...
            <div class="border-t border-slate-100 border-[#e7e5e4] pt-4 space-y-3">
              <p class="text-[10px] font-black uppercase tracking-wider text-[#44403c]">Folder Permission Rules</p>
              <div class="bg-slate-50 bg-white p-3.5 rounded-xl border border-[#e7e5e4]/70 space-y-2">
                <p class="text-[11px] text-slate-900 text-[#1c1917] font-bold">Restrict User Roles Access</p>
      ...
      <div *ngIf="showFolderModal()" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white bg-white border border-[#e7e5e4] p-6 rounded-2xl w-full max-w-sm space-y-4 text-xs animate-fadeIn">
          <h3 class="text-sm font-extrabold text-slate-800 text-[#1c1917]">Create Drive Folder</h3>
          <div>
            <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Folder Name</label>
            <input type="text" [(ngModel)]="newFolderName" placeholder="Sales Assets" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]">
          </div>
          <div>
            <label class="block text-[10px] font-bold text-[#44403c] uppercase mb-1">Folder Type</label>
            <select [(ngModel)]="newFolderType" class="w-full bg-slate-50 bg-white border border-slate-200 border-[#e7e5e4] rounded-xl px-3 py-2 text-slate-800 text-[#1c1917]">
      ...
      <div *ngIf="previewingFile()" class="fixed inset-0 bg-slate-900/75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl w-full max-w-lg space-y-4 text-xs animate-fadeIn text-slate-100 flex flex-col h-[400px]">
          <div class="flex justify-between items-center pb-2 border-b border-slate-800">
            ...
            <button (click)="previewingFile.set(null)" class="text-[#44403c] hover:text-white">
              <span class="material-icons">close</span>
            </button>
          </div>

          <!-- Simulated File Content Frame -->
          <div class="flex-1 bg-slate-950 border border-slate-900 p-6 rounded-xl overflow-y-auto font-mono text-[10px] text-[#44403c]">
