  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-extrabold text-slate-800 dark:text-white tracking-tight">Quotations & Invoicing</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Generate service agreements, calculate line item tax rates, and export PDF proposals.</p>
        </div>
      ...
      <!-- Main Directory list -->
      <div *ngIf="activeView() === 'list'" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-sm p-6 overflow-hidden animate-fadeIn">
        <h4 class="font-extrabold text-xs text-slate-400 uppercase tracking-widest mb-4">Contracts & Agreements</h4>
          ...
          <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-700 text-xs">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700 font-medium">
              <tr *ngFor="let doc of documents()" class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                <td class="py-4">
                  <span class="font-bold text-slate-800 dark:text-white text-sm cursor-pointer hover:text-indigo-500" (click)="openDocumentDetails(doc)">
                    {{ doc.name }}
                  </span>
                </td>
                <td class="py-4 uppercase text-[10px] text-slate-500">{{ doc.type }}</td>
                <td class="py-4 text-slate-600 dark:text-slate-300">{{ doc.customer?.companyName || 'Lead Account' }}</td>
                <td class="py-4 font-bold text-slate-800 dark:text-white">\${{ doc.metadata?.netAmount?.toLocaleString() }}</td>
                <td class="py-4 text-slate-400">v{{ doc.version }}</td>
                <td class="py-4">
                  <span [ngClass]="{
                    ...
                    'bg-slate-100 text-slate-500': doc.status === 'Draft'
                ...
                <td class="py-4">
                  ...
                  <span *ngIf="!doc.googleDriveLinked" class="text-slate-400 flex items-center gap-1">
              ...
              <tr *ngIf="documents().length === 0">
                <td colspan="8" class="text-center py-12 text-slate-400 font-semibold">No agreements designed yet.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div *ngIf="activeView() === 'builder'" class="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn">
        ...
        <div class="lg:col-span-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 p-6 rounded-2xl shadow-sm space-y-6 text-xs">
          <div class="flex justify-between items-center border-b pb-4">
            <h3 class="text-sm font-extrabold text-slate-800 dark:text-white">New Agreement Proposals</h3>
            <button (click)="setView('list')" class="text-slate-400 hover:text-slate-600"><span class="material-icons text-sm">close</span></button>
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Document Title</label>
              <input type="text" [(ngModel)]="newDocName" placeholder="GrownX Service SLA" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
            </div>
            <div>
              <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Document Type</label>
              <select [(ngModel)]="newDocType" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
          ...
          <!-- Customer selector -->
          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Select Customer Account</label>
            <select [(ngModel)]="selectedCustomerId" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
          ...
          <div class="space-y-4">
            <div class="flex justify-between items-center">
              <p class="font-extrabold text-[10px] uppercase tracking-wider text-slate-400">Line Items</p>
            ...
            <div *ngFor="let item of lineItems(); let idx = index" class="grid grid-cols-12 gap-3 items-center">
              <div class="col-span-6">
                <input type="text" [(ngModel)]="item.description" placeholder="Consulting Services" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
              </div>
              <div class="col-span-2">
                <input type="number" [(ngModel)]="item.quantity" (change)="calculateFinancials()" placeholder="1" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
              </div>
              <div class="col-span-3">
                <input type="number" [(ngModel)]="item.unitPrice" (change)="calculateFinancials()" placeholder="150" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
              </div>
        ...
        <div class="lg:col-span-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 p-6 rounded-2xl shadow-sm space-y-6 text-xs font-semibold text-slate-500 dark:text-slate-400">
          <h4 class="font-extrabold text-sm text-slate-755 dark:text-white uppercase tracking-wider">Financial Calculations</h4>
          
          <div class="space-y-4">
            <div>
              <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Tax Rate (%)</label>
              <input type="number" [(ngModel)]="taxRate" (change)="calculateFinancials()" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
            </div>
            
            <div>
              <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Discount Rate (%)</label>
              <input type="number" [(ngModel)]="discountRate" (change)="calculateFinancials()" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-800 dark:text-white">
            </div>

            <div class="border-t border-slate-100 dark:border-slate-700 pt-4 space-y-2.5">
              <div class="flex justify-between">
                <span>Subtotal:</span>
                <span class="text-slate-900 dark:text-white">\${{ subtotal }}</span>
              </div>
              ...
              <div class="flex justify-between pt-2 border-t dark:border-slate-700 font-bold text-slate-800 dark:text-white text-sm">
      ...
      <div *ngIf="selectedDoc()" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-800 border dark:border-slate-700 p-6 rounded-2xl w-full max-w-lg space-y-6 text-xs animate-fadeIn text-slate-700 dark:text-slate-200 flex flex-col h-[500px]">
          ...
          <div class="flex justify-between items-center border-b pb-3 shrink-0">
            <div>
              <h3 class="text-sm font-extrabold text-slate-800 dark:text-white">{{ selectedDoc().name }}</h3>
              <p class="text-[10px] text-slate-400 mt-0.5">Version: v{{ selectedDoc().version }} • Status: {{ selectedDoc().status }}</p>
            </div>
            <button (click)="selectedDoc.set(null)" class="text-slate-400 hover:text-slate-655">
          ...
          <div class="flex-1 overflow-y-auto space-y-6 pr-2">
            ...
            <!-- Line Items Table -->
            <div class="space-y-3">
              <p class="text-[10px] font-black uppercase text-slate-400">Line items details</p>
              <div class="space-y-2">
                <div *ngFor="let item of selectedDoc().metadata?.lineItems" class="flex justify-between bg-slate-50 dark:bg-slate-950 p-3 rounded-lg">
                  <span>{{ item.description }} (Qty {{ item.quantity }})</span>
                  <strong class="text-slate-800 dark:text-white">\${{ item.total }}</strong>
                </div>
              </div>
            </div>
            ...
            <!-- Signatures -->
            <div class="space-y-3">
              <p class="text-[10px] font-black uppercase text-slate-400">E-Signature Capture</p>
              
              <div *ngIf="selectedDoc().metadata?.signaturePng" class="border dark:border-slate-700/60 p-4 rounded-xl flex justify-center bg-slate-50 dark:bg-slate-950">
                <span class="material-icons text-emerald-400 text-4xl">verified</span>
                <span class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-2 ml-2">Signed Digitally via GrownX Portal</span>
              </div>
              
              <div *ngIf="!selectedDoc().metadata?.signaturePng" class="space-y-2">
                <p class="text-[10px] text-slate-400">Simulate signing this proposal agreement digitally.</p>
