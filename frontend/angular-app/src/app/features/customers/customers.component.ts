  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 class="text-2xl font-extrabold text-slate-800 text-[#1c1917] tracking-tight">Customer Directory</h1>
          <p class="text-sm text-[#574c43] mt-1">Manage corporate profiles, VIP accounts, and account assignments</p>
        </div>
      ...
      <!-- Filters Toolbar -->
      <div class="bg-white bg-white border border-slate-200 border-[#e7e5e4]/60 p-4 rounded-xl shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div class="relative w-full md:max-w-xs">
          ...
          <input 
            ...
            class="w-full pl-10 pr-4 py-2 border border-slate-200 border-[#e7e5e4] rounded-lg bg-slate-50 bg-[#fafaf9]/50 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 text-[#44403c]">
        </div>
        ...
        <div class="flex flex-wrap gap-3 w-full md:w-auto justify-end">
          ...
          <select 
            ...
            class="px-3 py-2 border border-slate-200 border-[#e7e5e4] rounded-lg bg-slate-50 bg-[#fafaf9]/50 text-xs text-[#44403c] focus:outline-none focus:ring-2 focus:ring-sky-500">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-sm">
              <tr *ngIf="customers().length === 0" class="text-center text-[#44403c] p-8">
              ...
              <tr *ngFor="let c of customers()" class="hover:bg-slate-50/50 dark:hover:bg-slate-700/10 transition-colors">
                ...
                <td class="px-6 py-4 font-bold text-slate-800 text-[#1c1917]">{{ c.companyName }}</td>
                <td class="px-6 py-4">
                  <div class="font-medium">{{ c.contactPerson }}</div>
                  <div class="text-xs text-[#44403c]">{{ c.email }}</div>
                </td>
                <td class="px-6 py-4 text-xs text-slate-600 dark:text-[#44403c]">{{ c.industry }}</td>
                <td class="px-6 py-4 text-xs font-medium text-slate-700 dark:text-[#1c1917]">
                  {{ c.assignedEmployee ? c.assignedEmployee.name : 'Unassigned' }}
                </td>
                <td class="px-6 py-4 font-semibold text-slate-800 text-[#1c1917]">
                ...
                <td class="px-6 py-4">
                  <span [ngClass]="{
                    ...
                    'bg-slate-100 text-slate-600 bg-[#fafaf9] dark:text-[#44403c]': c.status === 'Inactive'
                ...
                <td class="px-6 py-4 text-right text-xs font-semibold space-x-2">
                  ...
                  <button *ngIf="canModify()" (click)="openEditModal(c)" class="text-[#292524] hover:text-[#44403c]">Edit</button>
      ...
      <div *ngIf="isModalOpen()" class="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm">
        <div class="w-full max-w-lg bg-white bg-white h-full p-6 shadow-2xl overflow-y-auto flex flex-col justify-between">
          <div class="space-y-6">
            <div class="flex justify-between items-center border-b border-slate-200 border-[#e7e5e4] pb-4">
              <h3 class="text-lg font-bold text-slate-800 text-[#1c1917]">
            ...
            <!-- Modal Form -->
            <form class="space-y-4 text-[#44403c]">
              <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Company Name</label>
                <input type="text" [(ngModel)]="formModel.companyName" name="companyName" class="modal-input">
              </div>

              <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Contact Person</label>
                <input type="text" [(ngModel)]="formModel.contactPerson" name="contactPerson" class="modal-input">
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Email</label>
                  <input type="email" [(ngModel)]="formModel.email" name="email" class="modal-input">
                </div>
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Phone</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Industry</label>
                ...
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Status</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Revenue Generated</label>
                  <input type="number" [(ngModel)]="formModel.revenueGenerated" name="revenueGenerated" class="modal-input">
                </div>
                <div>
                  <label class="block text-xs font-semibold uppercase tracking-wider text-[#44403c]">Assigned Agent</label>
          ...
          <div class="flex gap-3 border-t border-slate-200 border-[#e7e5e4] pt-4 mt-6">
            <button (click)="closeModal()" class="flex-1 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-slate-700 text-[#1c1917] font-semibold text-sm">Cancel</button>
